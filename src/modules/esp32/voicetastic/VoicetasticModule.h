...
#error                                                                                                                           \
    "HAS_VOICETASTIC requires BOARD_HAS_PSRAM. Either define BOARD_HAS_PSRAM in the variant's platformio.ini, or unset HAS_VOICETASTIC."
#endif
...
class VoicetasticModule : public SinglePortModule, private concurrency::OSThread
{
  public:
    ...
    bool enqueueOutbound(const uint8_t *audio, size_t audio_len, voicetastic::CodecId codec, uint8_t codec_param,
    ...
    voicetastic::Codec2Mode getCodec2Mode() const
    {
        concurrency::LockGuard g(&state_lock_);
        return codec2_mode;
    }
    ...
    size_t pendingPlayCount() const
    {
        concurrency::LockGuard g(&state_lock_);
        return pending_play_queue.size();
    }
    ...
    {
        concurrency::LockGuard g(&state_lock_);
        return play_total_ms;
    }
    NodeNum playbackFromNode() const
    {
        concurrency::LockGuard g(&state_lock_);
        return play_from_node;
    }
    uint32_t playbackMessageId() const
    {
        concurrency::LockGuard g(&state_lock_);
        return play_message_id;
    }
    // Inspect a queued message without consuming it.
    bool peekPending(size_t index, NodeNum &from, uint32_t &message_id, uint32_t &approx_duration_ms) const;
    ...
    bool peekPendingFull(size_t index, NodeNum &from, NodeNum &to, uint8_t &channel, uint32_t &message_id,
  ...
  private:
    ...
    bool enqueueOutboundLocked(const uint8_t *audio, size_t audio_len, voicetastic::CodecId codec, uint8_t codec_param,
    ...
    // Whether this message is the final one of its recording session. Drives
    // the wire `last_in_stream` flag (set only on the very last frame of the
    // last message - see sendOneChunk). One-shot recordings are their own
    // session, so this is true; a future multi-message splitter sets it false
    // on every message but the last. Matches core's BuildConfig.last_in_stream.
    bool tx_last_in_session = true;
    ...
    uint8_t retransmitPop()
    {
        ...
        if (tx_retransmit_head > 16 && tx_retransmit_head * 2 >= tx_retransmit_idx.size()) {
            tx_retransmit_idx.erase(tx_retransmit_idx.begin(), tx_retransmit_idx.begin() + tx_retransmit_head);
            tx_retransmit_head = 0;
        }
        return v;
    }
