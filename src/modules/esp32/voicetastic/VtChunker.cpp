...
uint8_t chunkSizeForPreset(meshtastic_Config_LoRaConfig_ModemPreset preset)
{
    ...
    case P::meshtastic_Config_LoRaConfig_ModemPreset_SHORT_SLOW:
        return (uint8_t)MAX_BODY_SIZE; // 215 = 231 MTU - 16-byte header (was 219, which overran the MTU)
...
uint8_t defaultParityCount(uint8_t total_data)
{
    ...
    if (p < 1)
        p = 1;
    if (p > MAX_PARITY_PER_MESSAGE)
        p = MAX_PARITY_PER_MESSAGE;
    if ((int)total_data + (int)p > 255)
        p = (uint16_t)(255 - total_data);
    return (uint8_t)p;
}

bool buildOutbound(const uint8_t *audio, size_t audio_len, meshtastic_Config_LoRaConfig_ModemPreset preset, CodecId codec,
                   uint8_t codec_param, uint32_t message_id, uint8_t stream_seq, OutboundMessage &out)
{
    if (audio == nullptr || audio_len == 0)
        return false;

    const uint8_t chunk_size = chunkSizeForPreset(preset);
    if (chunk_size == 0)
        return false;
    if (audio_len > maxAudioBytes(chunk_size))
        return false;

    const size_t total_data_sz = (audio_len + chunk_size - 1) / chunk_size;
    if (total_data_sz == 0 || total_data_sz > MAX_CHUNKS_PER_MESSAGE)
        return false;
    ...
    if (parity_count > 0) {
        ...
        for (uint8_t i = 0; i < total_data; i++)
            dptrs[i] = out.data[i].data();
        for (uint8_t i = 0; i < parity_count; i++)
            pptrs[i] = out.parity[i].data();
        if (!rs::encode((int)total_data, (int)parity_count, (size_t)chunk_size, dptrs.data(), pptrs.data())) {
            return false;
        }
    }
    return true;
